...
func (tri *triangulator) triangulate() error {
	...
	//tri.triangleList = make([]Triangle, maxTriangles)//added wpl
	...
	//tri.triangleList = tri.triangleList[:tri.trianglesListLen]//?
...
func (t *triangulator) addTriangle(i0, i1, i2, a, b, c int) int {
	//the values coming in are indexes to nodes
	i := t.trianglesLen
	//j:= t.trianglesListLen
	t.triangles[i] = i0
	t.triangles[i+1] = i1
	t.triangles[i+2] = i2
	//t.triangleList[j] = Triangle{P1: t.points[i0], P2: t.points[i1], P3:t.points[i2]}
	t.link(i, a)
	t.link(i+1, b)
	t.link(i+2, c)
	t.trianglesLen += 3
	//t.trianglesListLen +=1
	return i
}
