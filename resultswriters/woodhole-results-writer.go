...
func InitwoodHoleResultsWriterFromFile(filepath string, frequencies []float64, discountFactor float64, analysisYear int, eeadWriter *WoodHoleEEADResultsWriter) (*WoodHoleResultsWriter, error) {
	...
	func() {
		...
		for _, val := range frequencies {
			...
			//fmt.Println(s)
			fieldDefsd := gdal.CreateFieldDefinition(sd, gdal.FieldType(gdal.FT_Real))
			defer fieldDefsd.Destroy()
			newLayer.CreateField(fieldDefsd, true)
			fieldDefcd := gdal.CreateFieldDefinition(cd, gdal.FieldType(gdal.FT_Real))
			defer fieldDefcd.Destroy()
			newLayer.CreateField(fieldDefcd, true)
			fieldDefd := gdal.CreateFieldDefinition(d, gdal.FieldType(gdal.FT_Real))
			defer fieldDefd.Destroy()
			newLayer.CreateField(fieldDefd, true)
			fieldDefw := gdal.CreateFieldDefinition(w, gdal.FieldType(gdal.FT_Real))
			defer fieldDefw.Destroy()
			newLayer.CreateField(fieldDefw, true)
		}
		fieldDefsead := gdal.CreateFieldDefinition(StructureFutureValueEAD, gdal.FieldType(gdal.FT_Real))
		defer fieldDefsead.Destroy()
		newLayer.CreateField(fieldDefsead, true)
		fieldDefcead := gdal.CreateFieldDefinition(ContentFutureValueEAD, gdal.FieldType(gdal.FT_Real))
		defer fieldDefcead.Destroy()
		newLayer.CreateField(fieldDefcead, true)
		fieldDefasead := gdal.CreateFieldDefinition(StructurePresentValueEAD, gdal.FieldType(gdal.FT_Real))
		defer fieldDefasead.Destroy()
		newLayer.CreateField(fieldDefasead, true)
		fieldDefacead := gdal.CreateFieldDefinition(ContentPresentValueEAD, gdal.FieldType(gdal.FT_Real))
